  ...
  return (
    <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md z-50 border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium text-slate-600">
          <button onClick={() => onNav("home")} className="hover:text-red-600">
            Home
          </button>
          <button onClick={scrollToAbout} className="hover:text-red-600">
            How it Works
          </button>
          <button
            onClick={() => onNav("predictor")}
            className="bg-slate-900 text-white px-6 py-2.5 rounded-full hover:bg-red-600"
          >
            Get Started
          </button>
        </div>
